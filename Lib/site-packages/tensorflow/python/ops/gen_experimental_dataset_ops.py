...
def assert_prev_dataset(input_dataset, transformations, output_types, output_shapes, name=None):
...
def data_service_dataset_v3(dataset_id, processing_mode, address, protocol, job_name, consumer_index, num_consumers, max_outstanding_requests, iteration_counter, output_types, output_shapes, uncompress_fn, task_refresh_interval_hint_ms=-1, data_transfer_protocol="", target_workers="AUTO", uncompress=False, name=None):
  r"""Creates a dataset that reads data from the tf.data service.
  ...
  if tld.is_eager:
    ...
    try:
      return data_service_dataset_v3_eager_fallback(
          ...
          uncompress_fn=uncompress_fn, name=name, ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
  # Add nodes to the TensorFlow graph.
  if not isinstance(output_types, (list, tuple)):
  ...
  _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "DataServiceDatasetV3", dataset_id=dataset_id,
                                ...
                                uncompress=uncompress, name=name)
  _result = _outputs[:]
  if _execute.must_record_gradient():
    _attrs = ("task_refresh_interval_hint_ms",
              _op._get_attr_int("task_refresh_interval_hint_ms"),
...
def data_service_dataset_v3_eager_fallback(dataset_id, processing_mode, address, protocol, job_name, consumer_index, num_consumers, max_outstanding_requests, iteration_counter, output_types, output_shapes, uncompress_fn, task_refresh_interval_hint_ms, data_transfer_protocol, target_workers, uncompress, name, ctx):
  if not isinstance(output_types, (list, tuple)):
    raise TypeError(
        "Expected list for 'output_types' argument to "
        "'data_service_dataset_v3' Op, not %r." % output_types)
